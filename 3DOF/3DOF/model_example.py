...
class DoubleGyre_Environment(gym.Env):
    ...
    def __init__(self, seed=None, lamb=0.02, rho=10, fix_init=None):
        ...
        self.action_space = spaces.Box(
            low=-self.max_action * np.ones(2, dtype=np.float32), high=self.max_action * np.ones(2, dtype=np.float32),
            dtype=np.float32
        )
    ...
    def _solve_ivp(self, action, store_traj):
        ...
        rhs = lambda t, y: self._get_v(t,
                                       y) + action  # if y[0] > 0 and y[0] < self.x and y[1] > 0 and y[1] < self.y else 0
