...
from PlotBoat import plot_series, plot_action_normal, create_animation
...
class Boat_Environment_PID(gym.Env):
    ...
    def step(self, action):
        ...
        environment[RUDDER_ANGLE] = self.controller.controll(self.ref_heading, x[YAW], x[YAW_RATE], speed,
                                                             0, drift_angle=drift)
        self.action_record.append(environment[RUDDER_ANGLE])
        #########
...
def main():
    action_dir = './model_output/PID动作记录'
    traj_dir = './model_output/PID轨迹图'
    state_array = []
    env = Boat_Environment_PID()
    state, _ = env.reset()
    temp_position = (env.state[0], env.state[1])
    state_array.append(temp_position)
    for i in range(env.N_steps):
        print(i)
        action = 0
        _, _, success, fail, test_info = env.step(action)
        if success:
            break
        temp_position = (env.state[0], env.state[1])
        state_array.append(temp_position)

    action_array = env.action_record
    plot_action_normal(action_array, save_dir=action_dir)
    plt.clf()
    _, _ = plot_series(state_array, end_point=env.end_target, title='trajectory Plot', xlabel='X/m',
                       ylabel='Y/m', label='trajectory',
                       legend=True, save_dir=traj_dir)
    plt.clf()
