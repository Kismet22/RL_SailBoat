...
path = np.array([[1., 1.04, 1.04, 1.05, 0.99, 0.93, 0.96, 1.02, 1.09, 1.05, 1.06, 1.06, 1.06, 1.06, \
                  1.15, 1.12, 1.19, 1.13, 1.17, 1.09, 1],
                 [0.8, 0.71, 0.61, 0.55, 0.47, 0.39, 0.46, 0.54, 0.59, 0.5, 0.56, \
...
def __array__(self, *args, **kwargs):
    import numpy as n
    if len(args) > 1 and isinstance(args[1], tuple) and isinstance(args[1][0], n.ufunc) and isinstance(args[1][0],
                                                                                                       n.ufunc) and len(
            args[1]) > 1 and args[1][0].nin == len(args[1][1]):
        if len(args[1][1]) == 3:
            raise Exception(
