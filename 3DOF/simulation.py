...
def calculate_sail_force(roll, wind, sail_angle):
    ...
    pressure = (AIR_DENSITY / 2) * wind.speed ** 2 * cos(roll * cos(sail_angle)) ** 2  # roll = 0, cos(roll * cos(
    # sail_angle)) ** 2 = 1
...
def solve(time, boat):
    ...
    """""""""
    # 当vel_z = 0时，pos_z不会发生改变
    print("boat_ROLL", boat[ROLL])
    print("boat_PITCH", boat[PITCH])
    print("boat_Z", (boat[POS_Z], boat[VEL_Z]))
    """
    ...
    delta_pos_z = vel_z  # vel_z = 0, delta_pos_z = 0

    # 角度一阶导数
    delta_roll = roll_rate  # roll_rate = 0
    delta_pitch = pitch_rate * cos(roll) - yaw_rate * sin(roll)  # roll = 0, delta_pitch = 0
    delta_yaw = yaw_rate * cos(roll) + pitch_rate * sin(roll)  # roll = 0, delta_yaw = yaw_rate
    ...
    delta = np.array(
        [delta_pos_x, delta_pos_y, delta_pos_z,
         ...
         delta_roll_rate, delta_pitch_rate, delta_yaw_rate], dtype=object)
    if actor_dynamics:
