...
def calculate_apparent_wind(yaw_in, vel_x, vel_y, true_wind):
    # 返回船体的视风
    ...
    # 此时以船前进方向作为坐标系
    # 将全局风速映射到前进坐标系上
    transformed_x = true_wind.x * ca.cos(yaw_in) + true_wind.y * ca.sin(yaw_in)

    # dy/dt
    transformed_y = true_wind.x * -ca.sin(yaw_in) + true_wind.y * ca.cos(yaw_in)
...
def calculate_sail_force_ca(wind_in, sail_angle_in):
    # 计算帆力
    ...
    '''
    # aoa : angle of attack
    aoa = wind_in['angle'] - sail_angle_in
    aoa = ca.if_else(aoa * sail_angle_in < 0, 0, aoa)

    # eff_aoa : effective angle of attack
    eff_aoa = ca.if_else(aoa < -ca.pi / 2, ca.pi + aoa, ca.if_else(aoa > ca.pi / 2, -ca.pi + aoa, aoa))

    pressure = (AIR_DENSITY / 2) * wind_in['speed'] ** 2  # roll = 0, cos(roll * cos(
    # sail_angle)) ** 2 = 1

    friction = ca.if_else(wind_in['speed'] != 0,
                          3.55 * ca.sqrt(AIR_VISCOSITY / (wind_in['speed'] * SAIL_LENGTH)),
                          0)

    separation = 1 - ca.exp(-((ca.fabs(eff_aoa)) / (ca.pi / 180 * 25)) ** 2)

    propulsion = (2 * ca.pi * eff_aoa * ca.sin(wind_in['angle']) - (
            friction + (4 * ca.pi * eff_aoa ** 2 * separation) / SAIL_STRETCHING) * ca.cos(wind_in['angle'])) \
                 * SAIL_AREA * pressure

    transverse_force = (-2 * ca.pi * eff_aoa * ca.cos(wind_in['angle'])
                        - (friction + (4 * ca.pi * eff_aoa ** 2 * separation) / SAIL_STRETCHING) * ca.sin(
                wind_in['angle'])) \
                       * SAIL_AREA * pressure

    separated_propulsion = sign_ca(aoa) * pressure * SAIL_AREA * ca.sin(aoa) ** 2 * ca.sin(sail_angle_in)
    separated_transverse_force = -sign_ca(aoa) * pressure * SAIL_AREA * ca.sin(aoa) ** 2 * ca.cos(sail_angle_in)
...
def shift_movement(T_in, t0_in, x0_in, u_in, f_in):
    # 运动到下一个状态
    f_value_out = f_in(x0_in, u_in[:, 0])
    st = x0_in + T_in * f_value_out
    # 时间增加
    t = t0_in + T_in
    # 准备下一个估计的最优控制，因为u[:, 0]已经采纳，我们就简单地把后面的结果提前
    u_end = ca.horzcat(u_in[:, 1:], u_in[:, -1])
    return t, st, u_end.T
...
xs = np.array([40.0, 20.0, np.nan, np.nan, np.nan, np.nan]).reshape(-1, 1)  # 末状态
...
def save_position_record_csv(position_record_in, file_name='./data/position_record.csv'):
    # 打开文件并写入数据
    with open(file_name, mode='w', newline='') as file:
        writer = csv.writer(file)
        # 写入每一行数据
        for record in position_record_in:
            writer.writerow(record)


save_position_record_csv(position_record)
