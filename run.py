...
def scenario_1(save=False):
    ...
    x2, t2, separation2, keel2, sail_force2, sail2, r2 = simulate(N_steps, x2, t2, r2, sail, environment, controller,
                                                                  integrator2, sampletime, sail_sampletime, ref_heading,
                                                                  wind, sail_angle)

    # comp_route(x, x2)

    plots_manoevers(t, x, r, sail, ref_heading, save, sail_force=sail_force, keel_force=keel, separation=separation,
                    wind=wind, append=append)
...
def plots_manoevers(t, x, r, sail, ref_heading, save, sail_force=None, keel_force=None, separation=None, wind=None,
                    append=""):
    # plots
    ...
    fig_heading, ax_heading = plot_time_fig(time=t, data=heading_data, labels=heading_labels, ax_labels=axlabels,
                                            scale=scale, ticks='ang')

    if save:
        fig_heading.savefig('figs/heading' + append + '.png')
        fig_ang.savefig('figs/angles' + append + '.png')
        fig_speed.savefig('figs/speeds' + append + '.png')
        fig_traj.savefig('figs/trajectories' + append + '.png')


def plot_time_fig(time, data, labels, ax_labels, scale=1., ticks=None):
    fig, ax = None, None
    for i, data_i in enumerate(data):
        fig, ax = plot_series(time, data_i * scale, label=labels[i], xlabel=ax_labels[0], ylabel=ax_labels[1], fig=fig,
                              ax=ax, legend=True)
...
def plot_old(t, x, r, sail, ref_heading, save, sail_force=None, keel_force=None, separation=None, wind=None,
                    append=""):
    ...
    fig_heading, ax_heading = plot_series(t, ref_heading / pi * 180, xlabel='time /s', ylabel='heading / deg',
                                          fig=fig_heading, ax=ax_heading)
    fig_heading, ax_heading = plot_series(t, drift / pi * 180, xlabel='time /s', ylabel='heading / deg',
                                          fig=fig_heading, ax=ax_heading)
...
def scenario_0(save=False):
    ...
    x, t, separation, keel, sail_force, sail, r = simulate(N_steps, x, t, r, sail, environment,
                                                           controller, integrator, sampletime, sail_sampletime,
...
def plot_arrows(x, y, directions, fig=None, ax=None, color=None):
    ...
    for i in range(x.shape[0]):
        ax.arrow(x[i], y[i], length * cos(directions[i]), length * sin(directions[i]), head_width=.05, head_length=.1,
                 fc=color, ec=color)


def plot_series(x, y, fig=None, ax=None, N_subplot=1, n_subplot=1, title=None, xlabel=None, ylabel=None, label=None,
                legend=False):
    if fig is None:
        fig = plt.figure()
    if ax is None:
