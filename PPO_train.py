...
################################### Training ###################################
def train(fix_init=None, K_epochs=50, update_const=20, lr_actor=0.005, lr_critic=0.01,  # RL hyperparameters
          agent='PPO', ckpt_id=None, random_seed=0, env_name='BoatEnvironment'):
    print("============================================================================================")
    ...
    env = Boat_Environment()  # seed=env_seed,
    ...
    # 一个episode的长度
    max_ep_len = env.t_end / env.sampletime - 2  # max timesteps in one episode
    ...
    # plot
    #########################
    fig_dir = agent + "_figs/" + env_name + '/' + str(fix_init)
    ...
    log_dir = agent + "_logs/" + env_name + '/' + str(fix_init)
    if not os.path.exists(log_dir):
        os.makedirs(log_dir)

    # create new log file for each run
    log_f_name = log_dir + agent + '_' + env_name + "_log_" + str(fix_init) + ".csv"
    log_f_full_name = log_dir + agent + "_" + env_name + "_log_" + str(fix_init) + (
        "_{}".format(ckpt_id) if ckpt_id is not None else "") + ".pkl"
    ...
    if ckpt_id is not None:
        ...
        checkpoint_path = directory + "{}_{}_{}_{}.pth".format(agent, env_name, fix_init, ckpt_id)
    else:
        checkpoint_path = directory + "{}_{}_{}.pth".format(agent, env_name, fix_init)
    ...
    if agent == 'PPO':
        ppo_agent = PPO(state_dim, action_dim, hidden_layer_dim, lr_actor, lr_critic, gamma, K_epochs, eps_clip, True,
                        action_std)
    ...
    while time_step <= max_training_timesteps:
        ...
        for t in range(1, int(max_ep_len + 1)):
            ...
            if time_step % print_freq == 0:
                ...
                plt.savefig("{}/running_path_{}.png".format(fig_dir, fix_init))
                """
        ...
        current_return = np.sum((env.state[:2] - env.end_target) ** 2)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--env_name', type=str, default='BoatEnvironment')
    args = parser.parse_args()
    print(args)

    if args.repeat > 0:
        for i in range(args.repeat):
            train(args.init, lr_actor=args.lr, lr_critic=args.lr * 2,
                  agent=args.agent, ckpt_id=i, random_seed=1234 + i, env_name=args.env_name)
    elif args.ckpt_id >= 0:
        train(args.init, lr_actor=args.lr, lr_critic=args.lr * 2,
              agent=args.agent, ckpt_id=args.ckpt_id, random_seed=1234 + args.ckpt_id, env_name=args.env_name)
    else:
        train(args.init, lr_actor=args.lr, lr_critic=args.lr * 2,
              agent=args.agent, env_name=args.env_name, K_epochs=100)
