...
from my_ppo_net import IH_PPO
from icm_ppo import ICM_PPO
from ppo import Classic_PPO
import os
import argparse
import torch
import numpy as np
...
def train(model_id, render_mode=None, checkpoint_path=None):
    ...
    # directory_1 = "HJB_preTrained"
    directory_1 = "IH_preTrained"
    ######################
    ...
    #                         has_continuous_action_space, action_std_init=action_std,
    ...
    # ppo_agent = IH_PPO(state_dim, action_dim, lr_actor, lr_critic, gamma, K_epochs, eps_clip,
    #                    has_continuous_action_space, delta_t=env.dt, action_std_init=action_std,
    #                    continuous_action_output_scale=action_output_scale,
    #                    continuous_action_output_bias=action_output_bias)  # hjb_lamda = 0.5 icm_alpha = 0.5
    ...
    #                     continuous_action_output_bias=action_output_bias, icm_alpha=0.5)
    ...
    while time_step <= max_training_timesteps:
        ...
        for t in range(1, max_ep_len + 1):
            ...
            if time_step % save_model_freq == 0:
                print("--------------------------------------------------------------------------------------------")
                checkpoint_path = directory_1 + f"timestep_{time_step}_std_{ppo_agent.action_std:.2f}.pth"
                print("saving model at : " + checkpoint_path)
                ppo_agent.save_full(checkpoint_path)
