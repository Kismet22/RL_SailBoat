...
class ActorCritic(nn.Module):
    def __init__(self, state_dim, action_dim, hidden_layer_dim, has_continuous_action_space, action_std_init,
        ...
        if has_continuous_action_space:
            self.actor = nn.Sequential(
                nn.Linear(lstm_hidden_dim, hidden_layer_dim),
                nn.Tanh(),
    ...
    def init_hidden(self, batch_size):
        """初始化 LSTM 的隐藏状态"""
        h_0 = torch.zeros(self.lstm_layers, batch_size, self.lstm_hidden_dim).to(device)
        c_0 = torch.zeros(self.lstm_layers, batch_size, self.lstm_hidden_dim).to(device)
        return (h_0, c_0)
    ...
    def act(self, state, hidden_state):
        ...
        :param hidden_state: LSTM 隐藏状态
        :return: 动作、动作的 log 概率、状态值、更新后的隐藏状态
        """
        batch_size = state.shape[0]
        state = state.unsqueeze(1)  # 添加时间维度 (batch_size, 1, state_dim)
        lstm_out, hidden_state = self.lstm(state, hidden_state)
        lstm_out = lstm_out[:, -1, :]  # 取最后的时间步 (batch_size, lstm_hidden_dim)
        ...
        return action.detach(), action_log_prob.detach(), state_val.detach(), hidden_state

    def evaluate(self, state, action, hidden_state):
        """
        评估 state-action pair
        :param state: (batch_size, state_dim)
        :param action: (batch_size, action_dim)
        :param hidden_state: LSTM 隐藏状态
        :return: action_log_probs, state_values, dist_entropy, 更新后的隐藏状态
        """
        batch_size = state.shape[0]
        state = state.unsqueeze(1)  # 添加时间维度 (batch_size, 1, state_dim)
        lstm_out, hidden_state = self.lstm(state, hidden_state)
        ...
        return action_log_probs, state_values, dist_entropy, hidden_state
