...
class HJB_PPO:
    ...
    def update(self):
        ...
        for _ in range(self.K_epochs):
            ...
            # print(f"delta_states_f shape: {delta_states_f.shape}")
            # print(f"state_gradients_f shape: {state_gradients_f.shape}")

            mse_f = torch.mean((state_values_f * log_gamma + rewards_f + transition_value) ** 2) # input:[n, state_dim]和[n,state_dim],自动广播成一个维度,output:标量
            ...
            # print(f"state_values shape: {state_values.shape}")
            # print(f"discount_rewards shape: {discount_rewards.shape}")

            mse_u = self.MseLoss(state_values, discount_rewards)  # input:[n]和[n,1],自动广播成一个维度,output:标量
            ...
            # print(f"old_state_values shape: {old_state_values.shape}")
