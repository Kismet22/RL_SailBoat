...
# 创建 PPO 模型
model = PPO("MlpPolicy", train_env, verbose=1, device="cuda", n_steps=640, learning_rate=0.001)

# 定义训练参数
total_episode = 2000
# 每次训练步长
timesteps_per_episode = 640
# 每5个episode保存一次数据
save_interval = 5
...
for episode in range(total_episode):
    ...
    print(action_array)
    print(state_array)
    mean_reward = np.sum(episode_rewards)
    print("episode_reward:", mean_reward)
    mean_rewards.append(mean_reward)

    # 每隔一定周期保存模型
    if episode % save_interval == 0:
        model.save(f"model2_{episode}.zip")
